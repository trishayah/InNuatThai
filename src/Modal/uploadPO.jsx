...
function UploadPO ({ setShowModal, onUploadSuccess }) {
  ...
  const handleUpload = async (event) => {
    ...
    if (selectedFile) {
      ...
      try {
        const response = await axios.post('http://localhost:3000/upload-po', formData, {
          headers: {
        ...
        const newDoc = {
          po_id: response.data.document.po_id,
          po_name: response.data.document.po_name,
          po_dateadded: response.data.document.po_dateadded,
        }
